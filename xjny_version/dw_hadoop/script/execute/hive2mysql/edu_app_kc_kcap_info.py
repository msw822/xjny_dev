...
filePath = "/tmp/xjny_daily/edu_app_kc_kcap/data" + data_day_str
...
if os.path.exists(filePath+"/000000_0") and os.path.getsize(filePath+"/000000_0") > 0:
    deleteShell = """mysql -h10.5.1.21  -P3306 -uroot -p123456 -e "TRUNCATE  app.edu_app_kc_kcap" """
    mysqlShell = """  mysql -h10.5.1.21  -P3306 -uroot -p123456 --local-infile=1 -e " SET NAMES utf8; LOAD DATA LOCAL INFILE '"""+filePath+"""/000000_0' into table app.edu_app_kc_kcap(kkxn,kkxq,kch,kcmc,kclx,xf,xs)  " """
    monitorShell = """mysql -h10.5.1.21 -P3306 -uroot -p123456 -e "insert into app.daily_report_data_monitor(date,table_name,complete_flag,create_time) values("""+data_day_int+""",'edu_app_kc_kcap',1,now()) " """
